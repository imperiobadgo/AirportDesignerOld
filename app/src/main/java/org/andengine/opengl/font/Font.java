...
//	public Font(final FontManager pFontManager, final ITexture pTexture, final Typeface pTypeface, final float pSize, final boolean pAntiAlias, final android.graphics.Color pColor) {
	...
	public synchronized void update(final GLState pGLState) {
		if (this.mTexture.isLoadedToHardware()) {
			...
			if (lettersPendingToBeDrawnToTexture.size() > 0) {
				...
				//this does not work
				//final android.graphics.PixelFormat pixelFormat = this.mTexture.getPixelFormat();
				// => workaround:
				...
				for (int i = lettersPendingToBeDrawnToTexture.size() - 1; i >= 0; i--) {
					...
					if (!letter.isWhitespace()) {
						...
						final boolean useDefaultAlignment = MathUtils.isPowerOfTwo(bitmap.getWidth()) && MathUtils.isPowerOfTwo(bitmap.getHeight()) && (pixelFormat == android.graphics.PixelFormat.RGBA_8888);
						...
						if (preMultipyAlpha) {
							...
							pGLState.glTexSubImage2D(GLES20.GL_TEXTURE_2D, 0, letter.mTextureX, letter.mTextureY, bitmap, this.mTexture.getPixelFormat());
						}
